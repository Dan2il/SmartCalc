...
int GetNumberOfPeriods(Periodicity periodicity, double deposit_term_in_month) {
  int result = 0;
  if (periodicity == LATE) {
    result = 1;
  } else if (periodicity == MONTHLY) {
    result = deposit_term_in_month;
  } else if (periodicity == QUARTERLY) {
    result = deposit_term_in_month / 3;
  } else if (periodicity == ANNUALLY) {
    result = deposit_term_in_month / 12;
  }
  return result;
}
