...
ErrorsType CheckCorrectBrackets(struct Expression *for_conversion) {
  ...
  if (for_conversion) {
    ...
    for (struct Node *node = FrontList(for_conversion->lexems); node != NULL;
         node = node->next_node) {
...
ErrorsType HandlerOperator(struct ConvertStation *converter) {
  ...
  if (CheckBrackets(&converter->handler_token->lexeme)) {
    errors = HandlerOperatorBrackets(converter);
             ...
             FrontList(converter->stack_operators)->lexeme.oprtr ==
                 STARTS_BRACKETS) {
...
ErrorsType HandlerComprassionOperators(struct ConvertStation *converter) {
  ...
  for (; operator->lexeme.oprtr != STARTS_BRACKETS && operator!= NULL &&
         errors == NO_ERRORS;) {
    if (converter->handler_token->lexeme.oprtr == POWER &&
        FrontList(converter->stack_operators)->lexeme.oprtr == POWER) {
      break;
    }
...
PriorityLvl GetPriorityLvl(struct Lexeme *lex) {
  ...
  if (lex->oprtr == ADDITION || lex->oprtr == SUBSTRACTION) {
    result = LOW;
  } else if (lex->oprtr == DIVISION || lex->oprtr == MULTIPLICATION ||
             lex->oprtr == MODULUS) {
    result = AVERAGE;
  } else if (lex->oprtr == POWER || lex->oprtr == UNARY_PLUS ||
             lex->oprtr == UNARY_MINUS || CheckFunctions(lex)) {
    result = HIGH;
  }
  return result;
}
...
bool CheckFunctions(struct Lexeme *lexeme) {
  bool result = false;
  if (lexeme->oprtr == COS || lexeme->oprtr == SIN || lexeme->oprtr == TAN ||
      lexeme->oprtr == ACOS || lexeme->oprtr == ASIN || lexeme->oprtr == ATAN ||
      lexeme->oprtr == SQRT || lexeme->oprtr == LN || lexeme->oprtr == LOG) {
    result = true;
  }
  return result;
}
